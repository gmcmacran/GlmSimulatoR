---
title: "Forward Step Wise Search"
...
With a multithreaded BLAS, forward step wise search provides a computationally light feature selection method. No resampling is needed, and features space is searched in an efficient way. In this vignette, this method will be tested in a variety of situations.
...
Using AIC, a forward search starts with no features. Then each feature is added. If there are 10 features, there are 10 models models under consideration. For each model, AIC is calculated and the model with the lowest AIC is selected. After the first feature is selected, all remaining 9 features are a considered. Of the 9 features, the one with the lowest AIC is selected, creating a 2 feature model. When adding no more features improves AIC, the procedure stops.
...
rm(simdata, scopeArg, glmSearch)


```

Looking at the summary, the correct model was found. Forward step wise search worked perfectly!
...
The correct model was found. Again, Forward step wise search worked perfectly!
...
### Summary
Forward step wise search provides a computationally fast way to select features. When the related features make up half the total possible features, step wise search performed perfectly for both small and large n. When there are a lot of unrelated features, forward step wise finds all related features and erroneously selects a few unrelated.
