...
test_that("Returns the correct range for x.", {
  expect_true(
    max(simulate_negative_binomial(weights = .5, xrange = 0)[, 2]) <= 1
  )
  expect_true(
    min(simulate_negative_binomial(weights = .5, xrange = 0)[, 2]) >= 1
  )
  expect_true(
    max(simulate_negative_binomial(weights = .5, xrange = 2)[, 2]) <= 3
  )
  expect_true(
    min(simulate_negative_binomial(weights = .5, xrange = 2)[, 2]) >= 1
  )
  expect_true(
    max(simulate_negative_binomial(weights = .5, xrange = 3)[, 2]) <= 4
  )
  expect_true(
    min(simulate_negative_binomial(weights = .5, xrange = 3)[, 2]) >= 1
  )
  expect_true(
    max(simulate_negative_binomial(weights = c(.5, 1), xrange = 0)[, 3]) <= 1
  )
  expect_true(
    min(simulate_negative_binomial(weights = c(.5, 1), xrange = 0)[, 3]) >= 1
  )
  expect_true(
    max(simulate_negative_binomial(weights = c(.5, 1), xrange = 2)[, 3]) <= 3
  )
  expect_true(
    min(simulate_negative_binomial(weights = c(.5, 1), xrange = 2)[, 3]) >= 1
  )
  expect_true(
    max(simulate_negative_binomial(weights = c(.5, 1), xrange = 3)[, 3]) <= 4
  )
  expect_true(
    min(simulate_negative_binomial(weights = c(.5, 1), xrange = 3)[, 3]) >= 1
  )
})
